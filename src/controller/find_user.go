...
import (
	...
	"github.com/JeremiasFernandes/PeDGo/src/view"
...
func (uc *userControllerInterface) FindUserByID(c *gin.Context) {
	logger.Info("Iniciando findUserByID controller",
	...
	if _, err := primitive.ObjectIDFromHex(userId); err != nil {
		logger.Error("Erro ao validar o userId",
			err,
			zap.String("journey", "findUserByID"),
		)
		errorMessage := rest_err.NewBadRequestError(
			"UserID nao eh um id valido",
		)
	...
	if err != nil {
		logger.Error("Erro ao chamar o findUserByID services",
			err,
			zap.String("journey", "findUserByID"),
		)
		c.JSON(400, err)
	...
	logger.Info("FindUserByID controller executado com sucesso",
		zap.String("journey", "findUserByID"),
	)
...
func (uc *userControllerInterface) FindUserByEmail(c *gin.Context) {
	logger.Info("Iniciando findUserByEmail controller",
	...
	if _, err := mail.ParseAddress(userEmail); err != nil {
		logger.Error("Erro ao validar userEmail",
			err,
			zap.String("journey", "findUserByEmail"),
		)
		errorMessage := rest_err.NewBadRequestError(
			"UserEmail nao eh um email valido",
		)
	...
	if err != nil {
		logger.Error("Erro ao chamar o findUserByEmail services",
			err,
			zap.String("journey", "findUserByEmail"),
		)
		c.JSON(err.Code, err)
	...
	logger.Info("findUserByEmail controller executado com sucesso",
		zap.String("journey", "findUserByEmail"),
	)
	c.JSON(http.StatusOK, view.ConvertDomainToResponse(
		userDomain,
