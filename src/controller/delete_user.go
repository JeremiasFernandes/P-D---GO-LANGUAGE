...
func (uc *userControllerInterface) DeleteUser(c *gin.Context) {
	...
	if _, err := primitive.ObjectIDFromHex(userId); err != nil {
		errRest := rest_err.NewBadRequestError("userId invalido, precisa ser um valor hexadecimal")
	...
	if err != nil {
		logger.Error(
			"Erro ao chamar o deleteUser service",
			err,
			zap.String("journey", "deleteUser"))
		c.JSON(err.Code, err)
		return
	...
	logger.Info(
		"deleteUser controller executado com sucesso",
